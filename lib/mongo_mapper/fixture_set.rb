...
require 'mongo_mapper/fixture_set/fixture'
require 'mongo_mapper/fixture_set/file'
require 'mongo_mapper/fixture_set/class_cache'
require 'mongo_mapper/fixture_set/test_helper'
...
module MongoMapper
  ...
  class FixtureSet
    ...
    class << self
      ...
      def create_fixtures(fixtures_directory, fixture_set_names, class_names = {})
        ...
        fixture_sets = files_to_read.map do |fs_name|
          ...
          fixtures_map[fs_name] = MongoMapper::FixtureSet.new(
                                      fs_name,
                                      klass,
      ...
      def sanitize_new_embedded_documents(document, is_new = false)
        document.associations.each do |name, relation|
          association = document.class.reflect_on_association(name)
          ...
          when :belongs_to
            if is_new && document.attributes[name]
              ...
              if value.is_a?(Hash)
                raise MongoMapper::FixtureSet::FixtureError.new "Unable to create nested document inside an embedded document"
              end
    ...
    def unmarshall_fixture(label, attributes, model_class)
      ...
      model_class.associations.each_value do |association|
        case association
        when MongoMapper::Plugins::Associations::BelongsToAssociation
          unmarshall_belongs_to(model_class, attributes, association)
        when MongoMapper::Plugins::Associations::ManyAssociation
          unmarshall_has_many(model_class, attributes, association)
        end
      end
    ...
    def unmarshall_belongs_to(model_class, attributes, relation)
      ...
      if value.is_a? Hash
        if relation.polymorphic?
          raise MongoMapper::FixtureSet::FixtureError.new "Unable to create document from nested attributes in a polymorphic relation"
        end
    ...
    def set_attributes_timestamps(model_class, attributes)
      ...
      if model_class < MongoMapper::Plugins::Timestamps
    ...
    def read_fixture_files
      ...
      yaml_files.each_with_object({}) do |file, fixtures|
        MongoMapper::FixtureSet::File.open(file) do |fh|
          fh.each do |fixture_name, row|
            fixtures[fixture_name] = MongoMapper::FixtureSet::Fixture.new(fixture_name, row, model_class)
          end
        end
      end
    end
